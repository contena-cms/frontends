 ...
 * updates. A CDN hit can replay a guest `ct-context-token` from when the entry
...
>(params: {
  ...
  const defaultHeaders = createHeaders(
    {
      "ct-access-key": params.accessToken,
      accept: "application/json",
      "ct-context-token": params.contextToken,
      ...params.defaultHeaders,
    },
    (key, value) => {
      apiClientHooks.callHook("onDefaultHeaderChanged", key, value);
      if (key === "ct-context-token" && value) {
  ...
  function createFetchClient(baseURL: string | undefined) {
    return ofetch.create({
      ...
      async onResponse(context) {
        ...
        // carry a stale guest ct-context-token from when the entry was stored.
        ...
        if (
          context.response.headers.has("ct-context-token") &&
          defaultHeaders["ct-context-token"] !==
            context.response.headers.get("ct-context-token")
        ) {
          const newContextToken = context.response.headers.get(
            "ct-context-token",
          ) as string;
          defaultHeaders["ct-context-token"] = newContextToken;
        }
      },
  ...
  return {
    ...
    updateBaseConfig: (config: { baseURL?: string; accessToken?: string }) => {
      ...
      if (
        ...
        defaultHeaders["ct-access-key"] = config.accessToken;
      }
