...
describe("createAPIClient", () => {
  ...
  it("should invoke requests with ct-access-key header and no context-token by default", async () => {
  ...
  it("should invoke requests with ct-context-token header", async () => {
    const firstAppSpy = vi.fn().mockImplementation(() => {});
    const app = createApp().use(
      "/checkout/cart",
      eventHandler(async (event) => {
    ...
    expect(firstAppSpy).toHaveBeenCalledWith(
      expect.objectContaining({
        "ct-access-key": "123",
        "ct-context-token": "456",
      }),
    );
  });

  it("should NOT invoke requests with ct-context-token header when not present", async () => {
    const seoUrlHeadersSpy = vi.fn().mockImplementation(() => {});
    const app = createApp().use(
      "/checkout/cart",
      eventHandler(async (event) => {
        const requestHeaders = getHeaders(event);
        seoUrlHeadersSpy(requestHeaders["ct-context-token"]);
        return {};
      }),
    );
  ...
  it("should invoke onContextChanged method when context token is changed", async () => {
    const app = createApp()
      .use(
        ...
        eventHandler(async (event) => {
          ...
          setHeader(
            event,
            "ct-context-token",
            String(requestHeaders["ct-context-token"]),
          );
          return {};
        }),
      )
      .use(
        "/context",
        eventHandler(async (event) => {
          setHeader(event, "ct-context-token", "789");
          return {};
        }),
      );
  ...
  it("should NOT adopt ct-context-token from publicly cacheable responses", async () => {
    // CacheableReads / CDN hits replay Cache-Control: public responses that may
    // still carry a guest ct-context-token from when the entry was stored.
    // Adopting that token would log the user out.
    const app = createApp().use(
      "/language",
      eventHandler(async (event) => {
        setHeader(event, "ct-context-token", "cached-guest-token");
        setHeader(
          event,
          "cache-control",
    ...
    expect(contextChangedMock).not.toHaveBeenCalled();
    expect(client.defaultHeaders["ct-context-token"]).toEqual(
      "logged-in-token",
    );
  });

  it("should still adopt ct-context-token from private session responses", async () => {
    const app = createApp().use(
      "/account/login",
      eventHandler(async (event) => {
        setHeader(event, "ct-context-token", "post-login-token");
        setHeader(
          event,
          "cache-control",
  ...
  it("should change default headers", async () => {
    ...
    client.defaultHeaders.apply({ "ct-language-id": "my-language-id" });
    await client.invoke("readCart get /checkout/cart");
  ...
  it("should apply headers from the request when overriding default one", async () => {
    ...
    await client.invoke("readCart get /checkout/cart", {
      headers: {
        "ct-language-id": "my-changed-language-id",
      },
    });
  ...
  describe("default header changes", () => {
    it("should invoke headers changed hook", async () => {
      ...
      await client.defaultHeaders.apply({ "ct-language-id": "my-language-id" });
      expect(client.defaultHeaders["ct-language-id"]).toEqual("my-language-id");
    ...
    it("context token headers change should invoke onContextChanged hook additionally", async () => {
      ...
      await client.defaultHeaders.apply({
        "ct-context-token": "some-new-context-token",
      });
