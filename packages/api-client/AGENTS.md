# AI Agent Guide for @contena/api-client

This document provides guidance for AI assistants working with the `@contena/api-client` package.

## TL;DR

**What**: Fully typed HTTP client for Contena 6 Store API and Admin API
**Tech**: TypeScript, ofetch, openapi-fetch-runtime
**Types**: Generated from OpenAPI spec via `@contena/api-gen` CLI
...
`@contena/api-client` is a thin, typed wrapper around [ofetch](https://github.com/unjs/ofetch). Its key feature is the `operations` generic that carries OpenAPI operation types end-to-end — from the `invoke()` call signature to the return type. Users either import the bundled default types or replace them with types generated from their own Contena instance via `@contena/api-gen`.
...
1. **Bundled default** — `@contena/api-client/store-api-types` or `@contena/api-client/admin-api-types`
2. **Generated from instance** — `./api-types/storeApiTypes` (via `@contena/api-gen`)
3. **Extended/overridden** — `./api-types/storeApiTypes.overrides.ts` (merges generated + custom)

When working on this package, never edit `api-types/*.d.ts` manually — they are generated artifacts. To regenerate them, use `@contena/api-gen` with the appropriate schema.
...
This runs `@contena/api-gen` against the configured Contena instance and regenerates the `.d.ts` files.
...
Tests live in `_tests/` and use Vitest with a mock HTTP server (no real Contena instance needed).
...
## Relationship with @contena/api-gen

`@contena/api-gen` is the CLI companion that produces the `operations` and `components` types consumed here. The two packages are decoupled — `api-client` ships default types for convenience, but end users are encouraged to generate their own.
...
- [@contena/api-gen](../api-gen/README.md) — Type generation CLI
- [developer.contena.cn/frontends](https://developer.contena.cn/frontends/) — Full documentation
